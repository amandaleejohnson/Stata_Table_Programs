capture program drop table1catcol
program define table1catcol
    ...
    if "`if'"=="" {
		putexcel set "table1_by`by'.xlsx", sheet("table1_col") modify
	}
	else {
		putexcel set "table1_by`by'_`if'.xlsx", sheet("table1_col") modify
	}	
	
	putexcel A1="Table 1. Column percentages of categorical variables by `by' `if'", bold
	putexcel B1="`by'"
...
foreach v of varlist `varlist' {
	...
	/*
	table1catcol new_race final_sex if session_condition==1, by(screener_own_wp) replace
